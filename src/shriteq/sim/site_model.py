...
class SiteModel:
    ...
    def step(
        ...
        fractions = [
            ...
            for value in (hvac_fraction, ev_fraction, pump_fraction)
        ]
        flexible_reduction_kw = sum(
            ...
            for fraction, bound in zip(
                fractions,
                (
                    self.config.hvac_flex_max_kw,
                    self.config.ev_flex_max_kw,
                    self.config.pump_flex_max_kw,
                ),
            )
        )
        ...
        delivered_ratio = (
            actual_shed_kw / flexible_reduction_kw if flexible_reduction_kw else 0.0
        )
        for name, fraction, bound in zip(
            ("hvac", "ev", "pump"),
            fractions,
            (
                self.config.hvac_flex_max_kw,
            ...
            self.deferred_energy_kwh[name] += (
                fraction * bound * delivered_ratio * dt_hours
            )
        ...
        soc, actual_charge_kw, actual_discharge_kw = self.battery.apply(
            charge_kw, discharge_kw, self.config.timestep_minutes / 60
        )
        grid_import_kw = max(
            0.0, net_load_kw - solar_kw - actual_discharge_kw + actual_charge_kw
        )
        solar_used_kwh = (
            min(max(0.0, solar_kw), net_load_kw + actual_charge_kw) * dt_hours
        )
        result = {
            ...
            "battery_throughput_kwh": (actual_charge_kw + actual_discharge_kw)
            * self.config.timestep_minutes
            / 60,
        }
        self._steps_in_day = (self._steps_in_day + 1) % (
            24 * 60 // self.config.timestep_minutes
        )
