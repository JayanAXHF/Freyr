...
class GridEdgeEnv(gym.Env):
    ...
    def _observation(self):
        end = self._position + self.horizon
        load = self.load_series.iloc[self._position:end].to_numpy()
        solar = self.solar_series.iloc[self._position:end].to_numpy()
        if len(load) < self.horizon:
            load = np.pad(load, (0, self.horizon - len(load)))
            solar = np.pad(solar, (0, self.horizon - len(solar)))
        forecast = np.column_stack((load, solar, np.full(self.horizon, 8.0))).astype(np.float32)
